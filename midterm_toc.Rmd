...
df2 <- df1 %>% select(age,sex,raceethn,provider,moa,mod,admtype,campus, los,target)
```
1. Use `filter` function to filter out rows where `raceethn==''` or `raceethn==''`. Make sure all the categorical variables are factor, numeric variables are numeric. Set Training : Testing Split = 10 : 90
...
```{r}
df1 <- df1 %>% rename(target=los)#new target(have not established anything else)
